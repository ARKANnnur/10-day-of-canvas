...
class Box {
  ...
  draw() {
    ...
    const geometryBullet = new THREE.SphereGeometry(0.5, 16, 16);
    const materialBullet = new THREE.MeshStandardMaterial({ color: 0xff0000 });
    const meshBullet = new THREE.Mesh(geometryBullet, materialBullet);

    return this.type === 'box' ? meshBox : meshBullet;
  }
  ...
  physic() {
    ...
    const shape = new CANNON.Sphere(0.5); // radius 0.5
    const bodyBullet = new CANNON.Body({
      mass: 1,
      shape,
      position: new CANNON.Vec3(), // akan di-set saat tembak
      material: new CANNON.Material(),
    });

    return this.type === 'box' ? bodyBox : bodyBullet;
  }
...
function shootSphere(event) {
  ...
  const bullet = new Box({ type: 'bullet' });
  scene.add(bullet.mesh);
  worldCannon.addBody(bullet.body);
  console.log(bullet);

  // 3. Set posisi bola
  bullet.body.position.copy(new CANNON.Vec3().copy(spawnPosition));
  bullet.body.velocity.set(0, 0, 0);
